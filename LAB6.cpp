...
double FindMinimums(double** matrix, int n, int m) {
    ...
    cout << "Последовательный режим.\tПрошедшло времени " << std::fixed << time << "\tРезультат " << otvet;
    cout << endl;
    return otvet;

}

double FindMinimumsSection_OLD(double** matrix, int n, int m, int count) {
    auto start = omp_get_wtime();
    int i, j;
    double minn = 1;
    double otvet = 1;
    ...
    masMin = new double[n];
    cout << endl;
#pragma omp parallel sections shared(matrix, n, m) private(i, j) num_threads(count)
    {
#pragma omp section
        {
            for (i = 0; i < n; i++) {
...
#pragma omp section
    ...
    auto end = omp_get_wtime();
    double time = end - start;
    cout << "Режим с Section.\tПрошедшло времени " << time << "\tРезультат " << otvet;
    cout << endl;
    return otvet;
...
double FindMinimumsParal(double** matrix, int n, int m, int count) {
    ...
    auto end = omp_get_wtime();
    double time = end - start;
    cout << "Параллельный режим\tПрошедшло времени " << time << "\tРезультат " << otvet;
    return otvet;

}
double** VvodMat(string filename, int n, int m) {
...
void Generator(int n, int m, string filename) {
...
int main()
{
    ...
    FindMinimums(matrix10, n10, m10);
    for (int i = 1; i <= MAX_THREADS; i++) {
        cout << "\nколичество потоков = " << i;
        FindMinimumsParal(matrix10, n10, m10, i);
        FindMinimumsSection(matrix10, n10, m10, i);
    }
    ...
    FindMinimums(matrix100, n100, m100);
    for (int i = 1; i <= MAX_THREADS; i++) {
        cout << "\nколичество потоков = " << i;
        FindMinimumsParal(matrix100, n100, m100, i);
        FindMinimumsSection(matrix100, n100, m100, i);
    }
    ...
    FindMinimums(matrix1000, n1000, m1000);
    for (int i = 1; i <= MAX_THREADS; i++) {
        cout << "\nколичество потоков = " << i;
        FindMinimumsParal(matrix1000, n1000, m1000, i);
        FindMinimumsSection(matrix1000, n1000, m1000, i);
    }
    ...
    FindMinimums(matrix10000, n10000, m10000);
    for (int i = 1; i <= MAX_THREADS; i++) {
        cout << "\nколичество потоков = " << i;
        FindMinimumsParal(matrix10000, n10000, m10000, i);
        FindMinimumsSection(matrix10000, n10000, m10000, i);
    }
